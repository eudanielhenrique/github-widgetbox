...
async function profileWidget(username, data, themeString) {
    ...
    function truncateName(name) {
        ...
        const availableWidth = width - 52 - dataBoxesWidth - NAME_START_X - NAME_RIGHT_MARGIN;
    ...
    try {
        ...
        return `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
                                    ...
                                    <g id="profile-card">
                                        ...
                                        <text id="text-name" fill="${theme.title}" data-name="text-name" transform="translate(145 78)" font-size="26" font-family="Roboto-Medium, Roboto, sans-serif" font-weight="500"><tspan x="0" y="0">${escapeXml(truncateName(response.data.name === null
            ? response.data.login
            : response.data.name))}</tspan></text>
                                        <text id="text-url" data-name="text-url" transform="translate(145 102)" fill="#bfbfbf" font-size="16" font-family="Roboto-Regular, Roboto, sans-serif"><tspan x="0" y="0">GitHub.com/${escapeXml(response.data.login)}</tspan></text>
